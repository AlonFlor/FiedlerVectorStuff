...
double run_one_sim_falling_constrained(char* inputFilename, int attempt_number, std::string constitutive_model, int numTimesteps){
   std::string output_file_prefix = inputFilename;
   std::string input_file = "models/";
   input_file += inputFilename;
   output_file_prefix = output_file_prefix.substr(0,output_file_prefix.size()-4) + "_" + constitutive_model + "_falling_constrained";
   ...
   int constrainedDOF_0 = get_start_index_of_vertex(2.00723, 2.20651, -0.614342, u_original, r);
   ...
   for(int i=0; i<numTimesteps; i++){
      ...
      // Yes, this means gravity is only applied as an initial force. I tried the version where gravity is constant, and it didn't work.
      implicitBackwardEulerSparse->SetExternalForcesToZero();
      if (i == 0){ // set some force at the first timestep
         for(int j=1; j<r; j+=3)f[j] = -9.8; // set gravity force for every node
